...
\placefigure[][fig:spreiding]
  ...
  {\starttikzpicture[scale=0.5]
   \draw[axis lines] (-0.2,0) -- (10.2,0) node[right] {$y$};
   \draw[axis lines] (0,-0.2) -- (0,5.2) node[above] {$I(y)$};
   \stoptikzpicture}
...
\section[sec:twee speleten,sec:postulaten]{Twee spleten}
...
\placefigure[][fig:spreidingen]
  ...
  {\starttikzpicture[scale=0.5]
   \draw[help lines] (0,0) -- (10,0) node[right] {$+$};
   \stoptikzpicture}{a}
  {\starttikzpicture[scale=0.5]
   \draw[axis lines] (-0.2,0) -- (10.2,0) node[right] {$y$};
   \draw[axis lines] (0,-0.2) -- (0,8.2) node[above] {$I(y)$};
   \stoptikzpicture}{b}
  \stopcombination
...
\placefigure[right][fig:phasor]
  ...
  {\starttikzpicture
   ...
   \draw[->] (0,0) -- node[auto] {$1$} (30:2);
   \draw[just lines] (1,0) arc[start angle=0,end angle=30,radius=1] node[below right] {$\phi$};
   \stoptikzpicture}
...
Dit is een \emph{vlakke golf} met amplitude $A$ en complexe fase
  ...
  \stopformula
Uitdrukkingen van de vorm $\e^{i \phi}$ noemen we een \emph{fasor}. Een fasor kunnen we weergeven in het complexe vlak als een eenheidsvector onder een hoek $\phi$ (zie \see[fig:phasor]).
...
\placefigure[][fig:acties]
  ...
  {\starttikzpicture[scale=0.5]
   ...
   \draw[axis lines] (-7.2,0) -- (7.7,0);
   \draw[axis lines] (-7,-0.2) -- (-7,6.7) node[above] {$\phi \sim S$};
...
%Elk pad $p_1,\dots,p_{14}$ heeft een bijbehorende fase $\phi_i$ gekoppeld aan de top van de golf zoals beredeneerd in \see[sec:fases]. De bijbehorende fasoren zijn getekend in \see[fig:som over paden]. Wanneer we al deze fasoren optellen, komen we uit op de fasor van het klassieke pad.  

In de vorige sectie hebben we gezien dat de actie een maat is voor de fase op het moment dat een deeltje zijn pad heeft doorlopen. Door bij elk pad een fasor op te stellen en deze te sommeren vinden we de waarschijnlijkheidsamplitude om van $B$ naar $D$ te komen. Dit is weergegeven in \see[fig:som over paden]. Hierbij vallen enkele dingen op:
\startitemize
...
\placefigure[][fig:som over paden]
  {}
  {\starttikzpicture
   ...
   % Teken nu alle pijlen door de coordinaten met elkaar te verbinden:
   ...
   % Verbind tot slot de twee eindpunten:
   \draw[emph lines,->] (P0) node[above right] {$B$} -- (P15) node[above] {$D$};
   \stoptikzpicture}

\section{Nog meer spleten}

We hebben nu genoeg informatie over padintegralen verzameld om een concrete afleiding te geven. Effectief lopen we de postulaten uit \see[sec:postulaten] in omgekeerde volgorde af. Let op.

\placefigure
  {}
  {\starttikzpicture[scale=0.7]
   \draw[axis lines] (-0.2,0) -- (10.2,0) node[right] {$x$};
   \draw[axis lines] (0,-0.2) -- (0,9.7) node[above] {$t$};
   \foreach \y in {1,...,9}
     \draw[help lines] (-0.1,\y) -- (10,\y);
   \draw[<->] (-0.5,5) -- node[left] {$\Delta t$} (-0.5,6);

   \coordinate[label=below left:$B$] (B) at (1,1);
   \coordinate[label=above right:$D$] (D) at (9,9);

   %\foreach \x [count=\n from 1] in {1,3,4,7,5,6,8,9}
     %\coordinate (P\n) at (\x,\n);

   \draw[just lines] plot[mark=*,mark options={orange}] coordinates {(B) (3,2) (4,3) (7,4) (5,5) (6,6) (8,7) (9,8) (D)};
   \stoptikzpicture}
...
Uit het derde postulaat weten we dat de \emph{bijdrage} aan een pad proportioneel is met de actie. Wanneer we van tijdsnede $n$ naar snede $n+1$ gaan kunnen we de bijdrage $\psi_{n \to n+1}$ uitrekenen met
  ...
  \stopformula
We moeten wel $S$ voor dit korte pad kunnen uitrekenen. Dit kan met een benadering van de Lagrangiaan tot op eerste orde:
  \startformula
  S \approx L\left( \frac{x_{n+1} + x_n}{2}, \frac{x_{n+1} - x_n}{\Delta t}, \frac{t_{n+1} + t_n}{2} \right) \Delta t.
  \stopformula
De bijdrage voor een stukje pad tussen tijdstip $t_n$ en $t_{n+1}$ is dan bij benadering
  \startformula
  \psi_{n \to n+1}
  \approx A \exp\left[ \frac{i}{\hslash} L\left( \frac{x_{n+1} + x_n}{2}, \frac{x_{n+1} - x_n}{\Delta t}, \frac{t_{n+1} + t_n}{2} \right) \Delta t \right].
  \stopformula
Maar we willen de bijdrage van het \emph{volledige} pad. Aangezien we in \see[sec:postulaten] ook hebben gezien dat we de bijdrage als een fase kunnen opvatten, krijgen we de totale fase door de fases van de afzonderlijke tijdsnedes te sommeren:
\todo{Iets met die constante}
  \startformula\startsplit
  \psi_{0 \to N} \SC
  \approx A \exp\left[ \frac{i}{\hslash} \sum_{n=0}^{N-1} L\left( \frac{x_{n+1} + x_n}{2}, \frac{x_{n+1} - x_n}{\Delta t}, \frac{t_{n+1} + t_n}{2} \right) \Delta t \right] \SR
  = A \prod_{n=0}^{N-1} \exp\left[ \frac{i}{\hslash} L\left( \frac{x_{n+1} + x_n}{2}, \frac{x_{n+1} - x_n}{\Delta t}, \frac{t_{n+1} + t_n}{2} \right) \Delta t \right].
  \stopsplit\stopformula
\todo{Moet later}
Om de correcte uitdrukking voor de bijdrage van één pad te krijgen, stappen we over op oneindig veel tijdsnedes. Met andere woorden: we nemen de limiet van $\Delta t$ naar $0$:
  \startformula
  \psi_{0 \to N}
  = \lim_{\Delta t \to 0} A \prod_{n=0}^{N-1} \exp\left[ \frac{i}{\hslash} L\left( \frac{x_{n+1} + x_n}{2}, \frac{x_{n+1} - x_n}{\Delta t}, \frac{t_{n+1} + t_n}{2} \right) \Delta t \right].
  \stopformula

Om de \emph{waarschijnlijkheidsamplitude} uit te rekenen bij de overgang van $B$ naar $D$ moeten we, volgens het tweede postulaat, de som nemen over \emph{alle mogelijke paden} van $B$ naar $D$. Tot nu toe hebben we alleen maar één pad uitgerekend. Daarbij zijn we er van uit gegaan dat ons pad langs specifieke punten $x_n$ loopt op tijdstip $t_n$. Deze plaats is natuurlijk willekeurig te kiezen uit \emph{alle mogelijke punten} $x$. Om elk mogelijk pad van $B$ naar $D$ te krijgen, kunnen we dus integreren over alle mogelijke tussen punten. Dit moeten we wel doen voor elke tijdsnede $t_n$ behalve voor $t_0$ en $t_N$. We krijgen dus $N-2$ keer een integraal over $x_n$ op tijdstip $t_n$ zodat
  \startformula
  \psi = \int \cdots \iint \psi_{0 \to N} \;\d x_1 \;\d x_2 \;\cdots \;\d x_{N-1}.
  \stopformula

\section{De Feynman padintegraal}
