...
\section[sec:twee speleten]{Twee spleten}
...
\placefigure[right][fig:phasor]
  ...
  {\starttikzpicture
   ...
   \draw[lines] (1,0) arc[start angle=0,end angle=30,radius=1] node[below right] {$\phi$};
   \stoptikzpicture}
...
\placefigure[][fig:acties]
  ...
  {\starttikzpicture[scale=0.5]
   ...
   \draw[alt lines] plot[samples at={-7,...,7},smooth,mark=*,mark options={orange}] (\x,{\x*\x/10+1});
   \stoptikzpicture}
...
\placefigure[][fig:som over paden]
  ...
  {\starttikzpicture
   ...
   \foreach \to [remember=\to as \from (initially 0)] in {1,...,15}
      \draw[->,alt lines] (P\from) -- node {$p_{\from}$} (P\to);
...
% vim: spell spl=nl cole=1 cocu=nvi
