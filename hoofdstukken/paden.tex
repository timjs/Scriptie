...
Om dit kwantitatief te maken, moeten we eerst een definitie geven van de actie. Deze wordt gegeven door een integraal van de Lagrangiaan van begintijdstip $t_{\text{b}}$ tot eindtijdstip $t_{\text{e}}$:
  \placeformula[for:actie definitie]
  \startformula
  S := \int_{t_{\text{b}}}^{t_{\text{e}}} L(x,\dot{x},t) \d t.
  \stopformula
Er zijn natuurlijk veel verschillende manieren om van een positie $x_b$ op tijdstip $t_{\text{b}}$ naar positie $x_e$ op tijdstip $t_{\text{e}}$ te komen. De positie $x$ in de Lagrangiaan is immers een functie van $t$ en kan vele vormen aannemen.
...
Van deze afwijking weten we dat
  ...
  \startformula
  \delta x(t_{\text{b}}) = \delta x(t_{\text{e}}) = 0,
  \stopformula
aangezien de begin- en eindpunten niet veranderen. Wanneer we willen dat de actie een minimum\footnote{Of eigenlijk een extremum.} heeft, wil dat zeggen dat $S$ niet verandert wanneer we het pad waarover we hem berekenen een klein beetje verandert. Dit heet het \emph{Variatieprincipe van Hamilton}. Op eerste orde geldt dan
...
Met andere woorden: de variatie in de actie, gegeven door de actie bij een alternatief pad minus de actie bij een klassiek pad, is nul. Voor de actie over een alternatief pad krijgen we
  \startformula
  S(\bar{x} + \delta x)
  = \int_{t_{\text{b}}}^{t_{\text{e}}} L(x + \delta x, \dot{x} + \delta \dot{x}, t) \d t.
  \stopformula
En wanneer we dit expanderen tot op eerste orde
  \startformula\startsplit
  S(\bar{x} + \delta x) \SC
  = \int_{t_{\text{b}}}^{t_{\text{e}}} \left( L(x, \dot{x}, t) + \delta \dot{x} \pdiff{L}{\dot{x}} + \delta x \pdiff{L}{x} \right) \d t \SR
  = S(x) + \int_{t_{\text{b}}}^{t_{\text{e}}} \left( \delta \dot{x} \pdiff{L}{\dot{x}} + \delta x \pdiff{L}{x} \right) \d t
  \stopsplit\stopformula
Dus voor de variatie in $S$ zoals gegeven in \see[for:variatie eis] geldt
  \startformula\startsplit
  \delta S \SC
  = \int_{t_{\text{b}}}^{t_{\text{e}}} \left( \delta \dot{x} \pdiff{L}{\dot{x}} + \delta x \pdiff{L}{x} \right) \d t \SR
  = \delta x \left. \pdiff{L}{\dot{x}} \right|_{t_{\text{b}}}^{t_{\text{e}}}
    - \int_{t_{\text{b}}}^{t_{\text{e}}} \delta x \left( \ddiff{t} \pdiff{L}{\dot{x}} - \pdiff{L}{x} \right) \d t
  = 0.
  \stopsplit\stopformula
...
We noemen een deeltje klassiek als zijn afmetingen groot zijn ten opzichte van een kwantummechanische lengteschaal. Hiervoor hanteren we de De~Broglie golflengte
  ...
  \lambda = \frac{h}{p}.
  \stopformula
Wanneer een deeltje een grootte heeft in de orde van een De~Broglie golflengte, mogen we kwantummechanische effecten \emph{niet} verwaarlozen.
...
\placefigure[][fig:spreidingen]
  {a. Spreiding door optellen van de losse spreidingen behorende bij spleet $S_1$ en spleet $S_2$. b. Spreiding zoals gemeten door de detector bij het twee-spleten experiment. De grijs gestreepte lijn geeft het diffractiepatroon weer bij twee spleten.}
...
De oplossingen van de Schrödingervergelijking zijn golven. Een superpositie van deze golven geeft ons een beschrijving van de evolutie van een deeltje. Voor een deeltje met gegeven impuls $p$ en totale energie $E$ heeft zo een golf de vorm
...
Wanneer we met een deeltje meereizen over zijn pad zal $\phi$ veranderen met een snelheid van
%Wanneer $\vec{r}$ de plaats aangeeft van de top van deze golf, dan wordt de faseverandering op dat punt gegeven door:
  ...
  = \left(m v^2 - \half m v^2 - V\right) / \hbar  \SR
  = \left(\half m v^2 - V\right) / \hbar  \SR
  = L / \hbar.
  \stopsplit\stopformula
...
Uit het derde postulaat weten we dat de \emph{bijdrage} aan een pad proportioneel is met de actie. Wanneer we van tijdsnede $n$ naar snede $n+1$ gaan kunnen we de bijdrage $\psi_{n \to n+1}$ uitrekenen met
  ...
  \startformula
  \psi_{n \to n+1} = A \e^{i S / \hbar}.
  \stopformula
Wel moeten we de actie voor dit korte pad kunnen uitrekenen. Dit kan met een benadering van de Lagrangiaan tot op eerste orde,
...
De bijdrage voor een stukje pad tussen tijdstippen $t_n$ en $t_{n+1}$ is dan bij benadering
  \startformula
  \psi_{n \to n+1} \approx A \exp\left[ \frac{i}{\hbar} \LApprox \Delta t \right].
  \stopformula
...
De gebeurtenissen $t_0, t_1, \dots, t_N$ vinden na elkaar plaats op hetzelfde pad. Stel je op tijdstip $t_0$ een fasor voor die langs de reële as ligt ($\phi_0=0$). We laten de fasor nu meereizen over het pad. Ondertussen draait hij rond met een snelheid die we in \see[for:faseverandering] hebben afgeleid. Wanneer hij is aangekomen op tijdstip $t_1$ is hij gedraaid met een hoek
  ...
  = A^N \prod_{n=0}^{N-1} \exp\left[ \frac{i}{\hbar} \LApprox \Delta t \right].
  \stopsplit\stopformula
...
Hierbij is de uitdrukking
  ...
  \startformula
  \int_{x_B}^{x_D} \D x := \lim_{N \to \infty} A^N \int_{-\infty}^\infty \cdots \int_{-\infty}^\infty \int_{-\infty}^\infty \d x_1 \d x_2 \;\cdots \d x_{N-1}
  \stopformula
...
Deze kernel is voor één dimensie en wordt volgens \see[for:padintegraal definitie] uitgerekend door
  \startformula
...
Wanneer we de maat uit \see[for:padintegraal maat] gebruiken en onze benaderingsmethode uit \see[for:benadering product] invullen krijgen we een lange reeks integralen. Effectief splitsen we de integraal over alle paden op in integralen over alle mogelijke tussenpunten:
  ...
  = \lim_{N \to \infty} A^N \int_{-\infty}^\infty \cdots \int_{-\infty}^\infty \int_{-\infty}^\infty 
...
Voor ons gemak hebben we de contstante $k := \frac{i m}{2 \hbar \Delta t}$ gedefinieerd. We hebben een integraal over een Gaussische functie die weer een Gaussische functie oplevert (zie ook \see[app:formules]). Met \see[for:exponent identiteit oneindig] krijgen we
  \startformula
  \frac{\sqrt{\pi}}{2\sqrt{2k}} \exp\left[ k \left( x_2^2 + x_0^2 - \frac{4 (x_2 + x_0)^2}{4 \cdot 2} \right) \right]
  \sim \exp\left[ \frac{k}{2} (x_2 - x_0)^2 \right].
  \stopformula
De constante factor kunnen we buiten beschouwing laten, aangezien we die kunnen absorberen in de constante $A$. De volgende exponentiële factor heeft de vorm
  \startformula
  \exp\left[ k (x_3 - x_2)^2 \right].
  \stopformula
We vermenigvuldigen ons resultaat met bovenstaande en integreren over $x_2$:
  \startformula\startsplit
  \int_{-\infty}^\infty \exp\left[ \frac{k}{2} (x_2 - x_0)^2 \right] \exp\left[ k (x_3 - x_2)^2 \right] \d x_2 \SC
  = \int_{-\infty}^\infty \exp\left[ \frac{k}{2} (x_2 - x_0)^2 + k (x_3 - x_2)^2 \right] \d x_2 \SR
  \sim \exp\left[ \frac{k}{3} (x_3 - x_0)^2 \right].
  \stopsplit\stopformula
We zien een recursief patroon ontstaan. Na in totaal $N-1$ keer integreren krijgen we een exponent in de vorm
  \startformula
  \exp\left[ \frac{k}{N} (x_N - x_0)^2 \right]
  = \exp\left[ \frac{i m}{2 \hbar N \Delta t} (x_N - x_0)^2 \right].
  \stopformula
We weten dat $x_0$ en $x_N$ respectievelijk onze begin- en eindpositie zijn, $x$ en $x'$ dus. Daarnaast volgt uit de definitie van $\Delta t$ in \see[for:delta t definitie] dat $N \Delta t = t - t'$. De kernel voor een vrij deeltje in één dimensie wordt dus
  \placeformula[for:kernel vrij deeltje]
  \startformula
  K(x,t;x',t') = A^N \exp\left[ \frac{i m}{2 \hbar (t - t')} (x - x')^2 \right].
  \stopformula
